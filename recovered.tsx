...
export default function ChampionshipDetailPage() {
  ...
  const fetchMyTeamEnrollment = () => {
    if (!token || !id) return;
    apiClient
      .get(`/championships/${id}/my-team-enrollment`)
      .then((data) => {
        setIsTeamEnrolled(!!data);
      })
      .catch((err) => console.error('Erro ao verificar inscrição da equipe', err));
  };

  const fetchTeamDashboard = () => {
    if (!token || !id) return;
    apiClient
      .get(`/championships/${id}/team-dashboard`)
      .then((data: any) => {
  ...
  const fetchAthleteDocument = () => {
    if (!token || !id) return;
    apiClient
      .get(`/championships/${id}/athlete-document`)
      .then((data: any) => {
        setAthleteDocument(data);
      })
      .catch((err) => console.error('Erro ao buscar documentos do atleta', err));
  };

  const handleEnrollTeam = async () => {
    if (!window.confirm('Tem certeza que deseja inscrever sua atlética neste campeonato?')) return;
    try {
      await apiClient.post(`/championships/${id}/enroll-team`);
      toast.success('Atlética inscrita no campeonato!');
      setIsTeamEnrolled(true);
    } catch (err: any) {
      toast.error(err.message || 'Erro ao inscrever atlética.');
    }
  };
  ...
  const toggleModality = (modId: string) => {
    ...
    const mod = champ?.modalities?.find((m: any) => m.id === modId);
    if (!mod) return;

    if (mod.type === 'COLETIVO' && athleteProfile.teamRole !== 'PRESIDENT') {
      toast.error('Apenas o Presidente da Equipe pode inscrever o time em modalidades coletivas.');
      return;
    }

    if (mod.type === 'INDIVIDUAL' && athleteProfile.teamRole !== 'PRESIDENT') {
      const isTeamInChampionship = mySubscriptions.some(sub => sub.modality?.championship?.id === id && sub.teamId === athleteProfile.team?.id);
      if (!isTeamInChampionship) {
        toast.error('Sua equipe precisa estar inscrita neste campeonato pelo presidente antes de você se inscrever em modalidades individuais.');
        return;
      }
    }
  ...
  const handleBulkSubscribe = async () => {
    ...
    if (athleteProfile.teamRole !== 'PRESIDENT') {
      toast.error('Apenas o Presidente da Equipe pode inscrever o time no campeonato.');
      return;
    }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-transparent pb-24 font-inter text-slate-200 pt-20">
        ...
        {/* TABS NAVIGATION */}
        <div className="bg-black border-b-2 border-neutral-800 sticky top-20 z-30">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                   ...
                   <p className="text-slate-600 text-sm mt-1">Faça login e vincule-se a uma equipe para se inscrever nas modalidades.</p>
                 </div>
              </div>
                          ...
                          <div className="mt-4 flex justify-between items-center gap-4">
                            {athleteProfile?.teamRole === 'PRESIDENT' ? (
                              ...
                              <>
                                {mod.type === 'COLETIVO' && (
                                  (() => {
                                    const inRoster = subscription.athletes?.some((a: any) => a.id === athleteProfile.id);
                                    if (inRoster) {
                                      return (
                                        <button 
                                          onClick={() => handleRemoveFromRoster(subscription.id, athleteProfile.id)}
                                          className="text-xs font-bold text-red-500 hover:text-red-700 hover:underline px-2 flex-1 text-center"
                                        >
                                          Sair do Elenco
                                        </button>
                                      );
                                      ...
                                      return (
                                        <button 
                                          onClick={() => handleAddToRoster(subscription.id, athleteProfile.id)}
                                          className="bg-green-600 text-white text-sm font-bold px-4 py-2 rounded-xl hover:bg-green-700 shadow-sm flex-1 text-center transition-colors"
                                        >
                                          Entrar no Elenco
                                        </button>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-8 overflow-y-auto bg-white flex-1 grid grid-cols-1 md:grid-cols-2 gap-8">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="font-bold text-slate-800 text-sm">
                                {member.user?.name} {member.user?.id === user?.id && <span className="text-blue-600 font-bold ml-1 text-xs">(você)</span>}
                              </p>
                              {isMemberAvailable && (
              ...
              {selectedSubscription && selectedSubscription.modality && (
