...
async function bootstrap() {
  ...
  const championshipRepo = app.get<Repository<Championship>>(
    getRepositoryToken(Championship),
  );
  const modalityRepo = app.get<Repository<Modality>>(
    getRepositoryToken(Modality),
  );
  ...
  const champ = championshipRepo.create({
    ...
    description:
      'O maior torneio do estado. Inscreva-se agora e represente sua equipe.',
  ...
  const modalities = [
    ...
    { name: 'Basquete', type: 'Misto', price: 15.0, championship: champ },
  ];
