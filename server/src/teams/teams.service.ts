  ...
  async updateDocumentStatus(
    ...
    if (data.type === 'rg') {
      ...
      if (data.status === 'REJECTED')
        profile.documentRgRejectionReason = data.rejectionReason || null;
      if (data.status === 'APPROVED') profile.documentRgRejectionReason = null;
    } else {
      profile.documentEnrollmentStatus = data.status;
      if (data.status === 'REJECTED')
        profile.documentEnrollmentRejectionReason =
          data.rejectionReason || null;
      if (data.status === 'APPROVED')
        profile.documentEnrollmentRejectionReason = null;
    }

    // Auto-update overall status
    if (
      profile.documentRgStatus === 'APPROVED' &&
      profile.documentEnrollmentStatus === 'APPROVED'
    ) {
      profile.status = 'APPROVED';
    } else if (
      profile.documentRgStatus === 'REJECTED' ||
      profile.documentEnrollmentStatus === 'REJECTED'
    ) {
